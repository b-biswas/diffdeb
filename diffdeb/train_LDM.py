...
from diffdeb.models import UNet, reparameterize, Encoder
from diffdeb.load_weights import load_model_weights
from diffdeb.train_UNet import train_step_UNet, eval_f_UNet


logging.basicConfig(level=logging.INFO)

@partial(jax.jit, static_argnames=['latent_dim', 'encoder_filters', 'encoder_kernels', 'dense_layer_units'])
def get_latent_images(params, batch, z_rng, latent_dim, encoder_filters, encoder_kernels, dense_layer_units):
...
def train_and_evaluate_LDM(
  ...
  for epoch in range(config.diffusion_config.num_epochs):
    ...
    for _ in range(config.diffusion_config.steps_per_epoch_train):
      ...
      state, batch_train_loss = train_step_UNet(state, (noisy_images, latent_batch), timestamps)
    ...
    if metrics["val loss"][epoch]<min_val_loss:
      ...
      ckpt = {'model': state, 'config': config.diffusion_config, 'metrics': metrics}
